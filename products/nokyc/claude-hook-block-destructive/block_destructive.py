...
RULES = [
    (r"\brm\s+(-[a-zA-Z]*[rR][a-zA-Z]*\s+-?[a-zA-Z]*[fF]|-[a-zA-Z]*[fF][a-zA-Z]*\s+-?[a-zA-Z]*[rR]|-[a-zA-Z]*(rf|fr)[a-zA-Z]*|--recursive\s+--force|--force\s+--recursive)\b[^;&|]*\s(/|~|\$HOME|\*|\.|\.\.)(\s|/?$|/\*)",
     "recursive force delete of root, home, cwd or wildcard"),
    ...
    try:
        ...
        with open(p, "a") as f:
            f.write(json.dumps({"ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                                "reason": why, "command": cmd, "cwd": data.get("cwd")}) + "\n")
    except Exception:
        pass
