...
- Verification: a payment counts as **verified** only once it is confirmed on-chain (RPC `eth_call balanceOf` / explorer transfer list). This container's egress proxy currently blocks every chain host (mainnet.base.org, base.blockscout.com, basescan.org, api.basescan.org), so all incoming payments stay **unverified** until the owner allows one of them.
